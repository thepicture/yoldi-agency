...
export const Input: React.FC<InputProps> & { Icon: typeof InputIcon } = ({
    children,
    placeholder = '',
    autocomplete = 'off',
    isInputVisible = true,
    ...
    return (
        <section className={styles.container}>
            <>
                ...
                />
                {!isInputVisible && text.length !== 0 && (
                    <input
                        className={[
                            styles.container__input_viewer,
                            isFocused
                                ? styles.container__input_viewer_focused
                                : void 0,
                        ].join(' ')}
                        placeholder={placeholder}
                        autoComplete="off"
                        disabled
                        value={
                            shouldRevealInput ? text : '*'.repeat(text.length)
                        }
                    />
                )}
                {!isInputVisible && (
                    <svg
                        ...
                        onClick={() =>
                            setShouldRevealInput((before) => !before)
                        }
                        onKeyDown={(event) =>
                            event.key === 'Enter' &&
                            setShouldRevealInput((before) => !before)
